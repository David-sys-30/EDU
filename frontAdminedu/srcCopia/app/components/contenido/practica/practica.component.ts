...
export class PracticaComponent implements OnInit {
	...
	ngOnInit() {
		this._contenidoService.obtenerTareas(this.idTema).subscribe(
			response=>{
				this.contenidoPractica = response.contenido;
	...
	openModificar(idContenido:string,
		...
		idEvaluacion:string,
		porcentaje:string){
		let dialogRef = this.dialog.open(ModificarPractica,{
			width: '800px',
			data:{
				contenido:{
	...
	fileChangeListener($event) {
		...
		if (this.fileList.length > 0) {
			...
			myReader.onloadend = (loadEvent: any) => {
				...
				this.practica = myReader.result;
				this.type = file.type;
				this.muestra = false;
				this.nombrePractica = file.name;
...
export class ModificarPractica{
	...
	constructor(private _contenidoService:ContenidoService,
		public dialogRef:MatDialogRef<ModificarPractica>,
