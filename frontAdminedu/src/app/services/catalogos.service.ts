...
export class CatalogosService{
	...
	bajaCategoria(idCategoria){
		let headers = new Headers({'Content-Type':'application/json'});
		return this._http.put(this.url+'bajaCategoria/'+idCategoria,{headers})
		.pipe(map(res => res.json()));
	}

	altaCategoria(idCategoria){
		let headers = new Headers({'Content-Type':'application/json'});
		return this._http.put(this.url+'altaCategoria/'+idCategoria,{headers})
		.pipe(map(res => res.json()));
	}
	// End Categorias
	...
	insertarEspecialidad(especialidad){
		...
		let params = json;
		let headers = new Headers({'Content-Type':'application/json'});
		return this._http.post(this.url+'insertaEspecialidad', params, {headers})
		.pipe(map(res=>res.json()));
	}

	getEspecialidades(){
		let headers = new Headers({'Content-Type':'application/json'});
		return this._http.get(this.url+'obtenerEspecialidades', {headers})
		.pipe(map(res=>res.json()));
	}

	modificarEspecialidad(especialidad){
		let json = JSON.stringify(especialidad);
		let params = json;
		let headers = new Headers({'Content-Type':'application/json'});
		return this._http.put(this.url+'modificaEspecialidad/'+especialidad.idCatalogoespecialidad, params, {headers})
		.pipe(map(res=>res.json()));
	}
