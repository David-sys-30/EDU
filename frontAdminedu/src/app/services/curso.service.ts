...
export class CursoService{
	...
	obtenerPersonaCursos(){
		let headers = new Headers({'Content-Type':'application/json'});
		return this._http.get(this.url+'getCursosSIns',{headers})
		.pipe(map(res=>res.json()));
	}
	...
	activacurso(idCurso){
		let idcurso = idCurso
		let headers = new Headers({'Content-Type':'application/json'});
		return this._http.put(this.url+'activarCurso/'+idcurso,{headers})
		.pipe(map(res=>res.json()));
	}

	desactivacurso(idCurso){
		let idcurso = idCurso
		let headers = new Headers({'Content-Type':'application/json'});
		return this._http.put(this.url+'desactivarCurso/'+idcurso,{headers})
		.pipe(map(res=>res.json()));
	}

	registraCalifCurso(CalifCurso,idUsuarioPersonaCurso){
		let json = JSON.stringify(CalifCurso);
		let params = json;
		let headers = new Headers({'Content-Type':'application/json'})
		return this._http.post(this.url+'registraCalifCurso/'+idUsuarioPersonaCurso,params,{headers})
		.pipe(map(res=>res.json()));
	}
